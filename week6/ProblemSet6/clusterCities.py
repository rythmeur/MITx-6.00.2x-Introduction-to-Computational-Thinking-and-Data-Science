...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for selfpoint in self.points:
            for otherpoint in other.points:
                if selfpoint.distance(otherpoint) < min_dist:
                    min_dist =  selfpoint.distance(otherpoint)
        return min_dist
    ...
    def maxLinkageDist(self, other):
        ...
        for selfpoint in self.points:
            for otherpoint in other.points:
                if selfpoint.distance(otherpoint) > max_dist:
                    max_dist =  selfpoint.distance(otherpoint)
        return max_dist
    def averageLinkageDist(self, other):
        ...
        for selfpoint in self.points:
            for otherpoint in other.points:
                counter+=1
                avrg_dist+= selfpoint.distance(otherpoint)
        return avrg_dist/counter*1.0
    def members(self):
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        # TO DO
        new_cluster = Cluster (c1.points+c2.points,c1.pointType)
    ...
    def findClosest(self, linkage):
        ...
        # print linkage(self.members[0], self.members[1])
        closest=()
        min_dist = 10**10
        for cluster1 in self.members:
            for cluster2 in self.members:
                if cluster1!=cluster2:
                    dist=linkage(cluster1, cluster2)
                    if dist < min_dist:
                        min_dist = dist
...
# print readCityData("cityTemps.txt", scale = True)
